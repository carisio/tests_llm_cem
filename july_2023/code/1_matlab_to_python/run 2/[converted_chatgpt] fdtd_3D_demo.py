...
Delta_s = min([Delta_x, Delta_y, Delta_z])  # Courant limit given by the smallest spatial step
...
for n in range(N_t):
    print(n)
    # Save Hz field in the center of the box
    t[n] = (n + 1) * Delta_t
    ...
    E_x[:, 1:N_y, 1:N_z] = E_x[:, 1:N_y, 1:N_z] + Delta_t / epsilon * (np.diff(H_z[:, :, 1:N_z], axis=2) / Delta_y - np.diff(H_y[:, 1:N_y, :], axis=1) / Delta_z)
...
k_exact = np.array([5.2359878, 7.55154489, 8.1788743, 8.9472598])  # rad/m
f_exact = k_exact * c / (2 * np.pi)  # Hz
H_z_f = np.fft.fft(H_z_t)
dummy = np.array([0, 0, 0, 0])

plt.plot(freq, np.abs(H_z_f[:N_t // 2]), 'b-', f_exact, dummy, 'x')
plt.show()
...
plt.plot(freq[1:end_pt] / 1e6, np.abs(H_z_f[1:end_pt]) / norm, 'b-')
f_exact /= 1e6
